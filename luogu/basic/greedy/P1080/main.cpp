...
struct Person {
    ...
    bool operator<(Person &p) {
        return left * right < p.left * p.right;
    }
} ministers[1005];
int main() {
    scanf("%d", &n);
    scanf("%d%d", &ministers[0].left, &ministers[0].right);
    for (int i=1; i<=n; ++i) {
        scanf("%d%d", &ministers[i].left, &ministers[i].right);
    }
    sort(ministers+1, ministers+1+n);
    BigIntTiny max_coin, coin;
    for (int i=1; i<=n; ++i) {
        coin=1;
        for (int j=i-1; j>=0; --j) {
            coin=coin * ministers[j].left;
        }
        coin=coin / ministers[i].right;
        if (coin>max_coin) max_coin = coin;
    }
    cout << max_coin.to_str();
}
